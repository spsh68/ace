var xml = {
    parseFromString: function(theString){
...
function plistToJson(el) {
    ...
    return $plistParse(el.querySelector("dict"));
};
...
function parseTheme(themeXml) {
	var el = xml.parseFromString(themeXml.trim()).documentElement
	if (el.tagName == "parsererror") {
		themeXml = themeXml.replace(/[\u0080-\uFFFF]/g, "").replace("\v","","g")
		var el = xml.parseFromString(themeXml.trim()).documentElement
	}
	//try {
        return plistToJson(el);
	//} catch(e) { return }   
}
...
function hyphenate(str) {
    return str.replace(/\s+(\w)/g, "-$1").replace(/[\s\-_]+([A-Z])/g, "-$1")
