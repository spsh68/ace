...
(function() {
    ...
    this.selectLine = function() {
        ...
        this.$moveSelection(function() {
            return this.moveCursorTo(rowEnd + 1, 0);
        });
    };

    this.moveCursorUp = function() {
        return this.moveCursorBy(-1, 0);
    };
    ...
    this.moveCursorLeft = function() {
        ...
        if (fold = this.session.getFoldAt(cursor.row, cursor.column, -1)) {
            ...
            if (cursor.row > 0) {
                return this.moveCursorTo(cursor.row - 1, this.doc.getLine(cursor.row - 1).length);
            }
        }
        else {
            var tabSize = this.session.getTabSize();
            if (this.session.isTabStop(cursor) && this.doc.getLine(cursor.row).slice(cursor.column-tabSize, cursor.column).split(" ").length-1 == tabSize)
                return this.moveCursorBy(0, -tabSize);
            else
                return this.moveCursorBy(0, -1);
        }
    };
    ...
    this.moveCursorRight = function() {
        ...
        if (fold = this.session.getFoldAt(cursor.row, cursor.column, 1)) {
            ...
            if (this.selectionLead.row < this.doc.getLength() - 1)
                return this.moveCursorTo(this.selectionLead.row + 1, 0);
        }
        else {
            var tabSize = this.session.getTabSize();
    ...
    this.moveCursorLineStart = function() {
        ...
        return this.moveCursorTo(startPos.row, startPos.column);
    };
    ...
    this.moveCursorBy = function(rows, chars) {
        ...
        return this.moveCursorTo(docPos.row, docPos.column + chars, chars == 0);
    };

    this.moveCursorToPosition = function(position) {
        return this.moveCursorTo(position.row, position.column);
    };
    ...
    this.moveCursorTo = function(row, column, preventUpdateDesiredColumn) {
        ...
        var moved = this.selectionLead.setPosition(row, column);
	...
	// sometimes after user input 
    this.dispatchCursorChange = function() {
