...
(function(){
    ...
    this.navigateUp = function(times) {
        this.selection.clearSelection();
        times = times || 1;
        if (!this.selection.moveCursorBy(-times, 0))
			this.selection.dispatchCursorChange();
    };
    ...
    this.navigateLeft = function(times) {
        ...
        if (!this.selection.isEmpty()) {
            ...
            moved = this.moveCursorToPosition(selectionStart)
        }
        else {
            times = times || 1;
            while (times--) {
                moved = moved || this.selection.moveCursorLeft();
            }
        }
        this.clearSelection();
		moved || this.selection.dispatchCursorChange();
    };
    ...
    this.navigateRight = function(times) {
        ...
        if (!this.selection.isEmpty()) {
            ...
            moved = this.moveCursorToPosition(selectionEnd)
        }
        else {
            times = times || 1;
            while (times--) {
                 moved = moved ||this.selection.moveCursorRight();
            }
        }
        this.clearSelection();
		moved || this.selection.dispatchCursorChange();
